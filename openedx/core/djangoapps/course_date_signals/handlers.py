...
def _get_custom_pacing_children(subsection):
    """
    ...
    while items:
        next_item = items.pop()
        num_weeks = next_item.fields['due_num_weeks'].read_from(next_item)
        if num_weeks:
...
def extract_dates_from_course(course):
    ...
    if course.self_paced and custom_pls_is_active(course.id):
        ...
        if SelfPacedRelativeDatesConfig.current(course_key=course.id).enabled:
            ...
            # apply the current PLS logic for self paced courses.
            for _, section, weeks_to_complete in spaced_out_sections(course):
                section_date_items = []
                for subsection in section.get_children():
                    if subsection.fields['due_num_weeks'].read_from(subsection):
                        section_date_items.extend(_get_custom_pacing_children(subsection))
                    else:
